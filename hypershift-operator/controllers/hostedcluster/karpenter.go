...
// reconcileTuningConfigSync mirrors Karpenter tuning ConfigMaps from the HostedCluster
// namespace into the hosted control plane namespace. Any ConfigMap in the HostedCluster
// namespace labeled hypershift.openshift.io/managed-by-karpenter with a tuning data key
// is copied into each Karpenter cluster's HCP namespace on reconcile.
...
func (r *HostedClusterReconciler) collectKarpenterTuningConfigMapNames(ctx context.Context, hcluster *hyperv1.HostedCluster) (set.Set[string], error) {
	...
	if err := r.List(ctx, configMapList, &client.ListOptions{
		...
		return nil, fmt.Errorf("failed to list karpenter tuning configmaps: %w", err)
	}
